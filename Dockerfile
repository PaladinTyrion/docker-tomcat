...
RUN set -ex \
  ...
  <role rolename="admin"/> \
  <role rolename="role1"/> \
  <user username="both" password="${PASSWD:-tomcat}" roles="tomcat,role1"/> \
  <user username="tomcat" password="${PASSWD:-tomcat}" roles="tomcat"/> \
  <user username="admin" password="${PASSWD:-admin}" roles="manager,admin,manager-gui,manager-script,manager-status,manager-jmx"/> \
  <user username="role1" password="${PASSWD:-tomcat}" roles="role1"/> \
</tomcat-users>' \
    && mkdir /logs \
